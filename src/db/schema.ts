...
}, (t) => [uniqueIndex('clerk_id_idx').on(t.clerId)])
...
export const subscriptions = pgTable("subscriptions", {
   viewer: uuid("viewer_id").references(() => users.id, { onDelete: "cascade" }).notNull(),
   creator: uuid("creator_id").references(() => users.id, { onDelete: "cascade" }).notNull(),
   createdAt: timestamp('created_at').defaultNow().notNull(),
   updatedAt: timestamp('updated_at').defaultNow().notNull()
}, (t) => [
    primaryKey({
...
export const vodepSelectSchema = createSelectSchema(videos)
...
export const relationTypes = pgEnum("relation_types", ["like", "dislike"])

export const videoReactions = pgTable("video_reactions", {
    userId: uuid("user_id").references(() => users.id, { onDelete: "cascade" }).notNull(),
    videoId: uuid("video_id").references(() => videos.id, { onDelete: "cascade" }).notNull(),
    type: relationTypes("type").notNull(),
    createdAt: timestamp('created_at').defaultNow().notNull(),
    updatedAt: timestamp('updated_at').defaultNow().notNull()
}, (t) => [
    primaryKey({
